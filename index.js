...
// const getSerie = () => {
//     let nomeSerie = serieNomeRef.value; // Pega o valor digitado pelo usuário no input
//     let url = `http://www.omdbapi.com/?t=${nomeSerie}&apikey=${key}`; // Constrói a URL da API

//     if (nomeSerie.length <= 0) {
//         resultado.innerHTML = <h3 class="msg"> </h3>; // Se o usuário não digitou nada, exibe uma mensagem vazia
//     } else {
//         fetch(url) // Faz a chamada à API usando fetch
//             .then((resp) => resp.json()) // Converte a resposta para JSON
//             .then((data) => {
//                 console.log("Resposta da API:", data);
                
//                 if (data.Response === "True") { // Se a API retornar um filme/série
//                     serie = {
//                         title: data.Title, // Armazena o título
//                         poster: data.Poster, // Armazena o poster
//                         status: statusSeries.value, // Armazena o status selecionado pelo usuário
//                     };

//                     // Exibe os detalhes da série no elemento com ID "filme-pesquisado"
...
//                         </div>
//                     `;
//                     resultado.innerHTML = ""; // Limpa qualquer mensagem de erro anterior

//                 } else {
//                     resultado.innerHTML = <h3 class="msg">${data.Error}</h3>; // Se a API retornar um erro, exibe a mensagem
//                 }
//             })
//             .catch((error) => { // Captura qualquer erro na requisição
